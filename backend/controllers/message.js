...
const gitAllMessage = (req, res) => {
  ...
  const persion_id = req.params.id;
  const query = `SELECT * FROM messages INNER JOIN users ON messages.persion_id = users.idUser WHERE messages.is_deleted=0 AND messages.user_id =? AND messages.persion_id =?`;
